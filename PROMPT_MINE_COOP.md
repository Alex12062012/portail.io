Contexte : tu as précédemment créé "game-portal" (portail JOLT.io, Poki-like) avec page
d'accueil + pseudo (`public/index.html`, `public/portal.js`), système de salons génériques
à code 4 lettres (`server/rooms.js` + registre `GAMES` dans `server/server.js`), et le jeu
"Tag Arena" (`server/games/tag-arena.js` côté serveur, `public/games/tag-arena/` côté
client, constantes/physique partagées dans `shared/tag-map.js` et `shared/tag-physics.js`
servies en statique sous `/shared/`). Je veux ajouter un deuxième jeu, "Mine Coop", dans ce
même projet, SANS casser Tag Arena ni le portail.
...
- Type de jeu `'mine-coop'` dans `GAMES`, réutilisant `room:create` / `room:join` /
  ...
  réutilisation du même flow que Tag Arena) : "Créer un monde" (crée un salon → code 4
  lettres + seed aléatoire générée côté serveur à la création de la partie) et "Rejoindre"
  (saisie du code). Jusqu'à 4 joueurs par salon (`minPlayers: 1, maxPlayers: 4` dans
  `GAMES['mine-coop']` — un monde solo doit pouvoir démarrer).
- Les pseudos (gérés par le portail via `localStorage('jolt.name')`, transmis comme pour
...
- Inventaire : hotbar de 9 emplacements + inventaire étendu, empilement des ressources
  (limite raisonnable par pile, ex. 99).
...
- Si un salon est recréé/rejoint avec le même code, recharge le monde sauvegardé tel quel.
- Limite : 5 mondes max sur disque. Une 6e sauvegarde supprime automatiquement la moins
  récemment utilisée (LRU, basé sur la date de dernière sauvegarde/accès). Documente ce
  comportement dans le README.
...
2. La page d'accueil affiche 2 cartes actives : Tag Arena et Mine Coop.
