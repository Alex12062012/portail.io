# Prompt combiné : Renommage portail.io + Inventaire, Craft, Coffre, ESC, Stats

Deux tâches à réaliser dans la même session. Fais-les dans l'ordre : **d'abord le renommage** (rapide, peu de fichiers), **ensuite l'inventaire/craft** (plus long). `npm test` doit passer à la fin.
...
# Partie 2 — Mine Coop : Inventaire, Craft, Coffre, ESC, Stats

Le portail s'appelle désormais **portail.io** (renommage fait en Partie 1). Lit les fichiers indiqués avant d'écrire une ligne. Le back-end et le client v1 existent déjà. Cette partie étend le jeu sans casser Tag Arena ni les tests.
